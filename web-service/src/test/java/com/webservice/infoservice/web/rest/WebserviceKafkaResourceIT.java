...
import com.webservice.infoservice.config.EmbeddedKafka;
import com.webservice.infoservice.config.KafkaSseConsumer;
import com.webservice.infoservice.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class WebserviceKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/webservice-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/webservice-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/webservice-kafka/unregister"));
                return;
            }
        }
